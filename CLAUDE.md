...
- **Hash index build:** `scripts/build-index.mjs` pulls catalogs from Pokemon TCG API (English, `--source ptcg`), TCGdex (`--source tcgdex`, JA/KO/ZH/DE/FR/IT/ES), and the official JP card DB pokemon-card.com (`--source jpofficial`, always ja; XY era → present; enumerates resultAPI.php, per-card details.php for the collection number, incremental via `srcId`), downloads card images, computes hashes, emits `index-<lang>.json` + `hashes-<lang>.bin` (merge is incremental; known ids skipped). Nightly GitHub Action (`refresh-index.yml`) maintains the full EN catalog, full JA SV era (TCGdex has no JA data before ~2022), and JA SM/SWSH era via jpofficial (`--match '^S(?!V)'`).
- **Korean cards:** no per-language KO index exists (TCGdex has zero KO card data; official KO sites geo-block non-KR IPs with 410). KO scans work because Korean sets mirror Japanese sets 1:1 (same art): scans match their JA twins, and KO-only rainbow-rare treatments match their EN twins (verified: KO Cheryl HR → EN swsh5-173 beats noise floor). Caveat: KO secret-rare numbering can differ from the JA print (KO 087/070 vs JA 081/070), and prices shown are for the JA/EN print, not the KO one.
